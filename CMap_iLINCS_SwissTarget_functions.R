...
multi_heatmaps_SwissCMapILINCS=function(swisstargetDir='swisstarget', iLINCSconnectivityDir='../ilincs/', cmapConnectDir='../cmap/',singleAnnotationTag='Measurements', annotationLegendTitle='user input logFC data',refdata=NULL, candygenes, swissFocus=T, CellLine='HA1E', secondCellLine=NULL, summaryDATA=F, nacols=NULL,width_heat=20, height_heat=14, res_heat=600){
  ...
  if(!is.null(summaryDATA)){gridheight=3
  }else{gridheight=1.5}
  #Generate heatmaps
  ...
  ht1=Heatmap(t(stp_tags$dataset),cluster_rows = F,  col = as.function(stp_tags$f1), name= "SwissTarget similarity scores",top_annotation=ha,row_names_gp = gpar(fontsize = 15, fontface = "bold", col=cols),column_names_gp = gpar(fontsize = 15, fontface = "bold"),
              heatmap_legend_param=list(legend_direction="horizontal",grid_height = unit(0.3, "cm"),grid_width = unit(0.3, "cm"),labels_gp = gpar(fontsize = 7),title_gp = gpar(fontsize = 7, fontface = "bold")),cluster_columns=F,row_title = "SwissTargetPrediction", row_title_rot = 0,row_title_gp = gpar(fontsize=20,fontface='bold', fill = "red", col = "white", border = "blue"), na_col = nacols, height = gridheight,use_raster=F); if(stp_tags$f1tag){ht1@matrix_legend_param$at=stp_tags$at; ht1@matrix_legend_param$labels=stp_tags$labels}
