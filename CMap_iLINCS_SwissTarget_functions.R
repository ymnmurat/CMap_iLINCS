...
##connectivitydatafile_compiler: Compiles CMap connectivity data.Currently, it works locally and utilizes names of the downloaded files to parse through. Files naming convention contains '_conn_CellLine_' (here _conn_HA1E_ by default - please check the file names in cmap folder). Main output file contains gene connectivity data from knock-down/over-expression studies. 
...
connectivitydatafile_compiler=function(ptrn=NULL, types=NULL,cormethod='spearman', cmaprank_cutoff=98, cor_outputs=F, heatmap_outputs=F, width_heat=18, height_heat=12, res_heat=300){
  ...
  require(dplyr)
  #all directions kd:knockdown, oe:overexpression, cp:compounds, cc:compound classes. If the input for the direction is missing, it will be kd and oe only for the consecutive analyses.
  ...
  for (ds in typewise) {
    ...
    #assign color codes for groups of compounds/classes(pro-coagulants/anti-coagulants)
    ...
    if (cor_outputs) {
      par(cex.main=1)
      lgd_list=Legend(labels=c("Pro-coagulants", "Anti-coagulants", 'CMap: Pro-coagulants','CMap: Anti-coagulants'), title="Compound types", type="points", pch=18, legend_gp=gpar(col=c("red4","black",'red', 'dimgray'), fontsize=24))
...
##Further developments by online queries are needed. So that, connectivity information would be collected for specified compounds, exposure times and concentrations
...
##iLINCS_DEG_connectivitydatafile_compiler: Similar to connectivitydatafile_compiler function, and compiles data from iLINCS instead.
...
swisstargetdatafile_compiler=function(ptrn=NULL,startRow=2, clippedSwissData=F, targetscore_cutoff=0.5, focused=F, focuscandies=NULL, focuscandiesThreshold=0.2,heatmap_outputs=F, clustering_method='ward.D',width_heat=18, height_heat=18, res_heat=300){
  ...
  ## Some targets contain multiple common names delimite by a space. We can produce new rows by splitting them.
...
slicenricher=function(enrichrlist, direction=NULL, combscore_cutoff=200, refcompounds=NULL, cmapranks=F,cormethod='spearman',enrichmentdb=NULL, cor_outputs=T, heatmap_outputs=T, enrichmentTOOL='EnrichR', width_heat=18, height_heat=12, res_heat=300){
  ...
  tryCatch({if (!cmapranks) {drugnames=gsub('\\.','_',gsub('.*_','',gsub('_[^_]*$','',colnames(as.data.frame(enrichrlist[[1]][1])))))
    ...
    proco_thr=c("heparin","thrombin",'phylloquinone', 'menadione','desmopressin','U46619')
    bloods=c('blood', 'thromb', 'agula', 'erythroc', 'leukoc', 'arteri','vein ', 'vascular')
...
##multi_heatmaps_SwissCMapILINCS compiles data SwissTargetPredcition, iLINCS and CMap derived data, and produces a combined heatmap figure from these data and a query list of genes. Optionally, users can provide a reference data (RNAseq based logFC, gene-phenotype correlation scores etc) as an annotation for the figure. Please, use similar file name conventions and directory allocations (see folders; swisstarget, ilincs, cmap under github/cmap_ilincs directory). CellLine is by default HA1E, you can additionally include secondCellLine argument. Optionally, you can provide a refdata to annotate your heatmaps. Final heatmaps are saved in main  github/cmap_ilincs directory automatically. Here you can also change the name for your single column reference data annotation on the heatmap, colors of the NA data (default 'gray') and width/height/resolution of your heatmap figureCompiles files that were retrieved from SwissTargetPrediction, CMap Conn and iLINCS platforms
multi_heatmaps_SwissCMapILINCS=function(swisstargetDir='swisstarget', iLINCSconnectivityDir='../ilincs/', cmapConnectDir='../cmap/',singleAnnotationTag='Measurements', annotationLegendTitle='user input logFC data',refdata=NULL, candygenes, swissFocus=T, CellLine='HA1E', secondCellLine=NULL, nacols=NULL,width_heat=20, height_heat=14, res_heat=600){
  ...
  #If provided, modify the reference data based on the list of query genes (candygenes)
  ...
  #Retrieve and compile iLINCS connectivity collections for the query genes. S
  ...
  #If provided, do the same CMap connectivity compilations for the second cell line as well
  if (!is.null(secondCellLine)) {
    rm( list = Filter( exists, c('dataset') ) )
  ...
  ##heatmaptags function: Setting the color scales and adjusting heatmap legends can become redundant and tricky. Especially, if any of the compiled data does not contain viable numbers but all NAs, we can run into some issues. heatmaptags function mainly handles the reducndancy for seeting the color scales and produces reference lists for us to handle potentially problematic cases. For example, if whole set of the compiled data is NA, it will convert them to 0s, assign a color scale of grays between a minimal range c(0,0.0001) as labels, include a flag tag to update the heatmap legend in the follow up for this type of cases and store them in a list. Otherwise, it will keep the data as it is, assign a color range of choice ('reds', 'blues') and keep the flag silent (F) to not to update the legend with NAs. 
  ...
  #assign labels for the compound names in case they are pro-coagulant, and make a legend for them
  ...
  if (!is.null(conndata_stp2)|!is.null(conndata_cmap_kd_stp2)|!is.null(conndata_cmap_oe_stp2)) { 
    cols_ilincs_Con2=rep('black', ncol(conndata_stp2))
    cols_cmap_kd_Con2=rep('black', ncol(conndata_cmap_kd_stp2))
    cols_cmap_oe_Con2=rep('black', ncol(conndata_cmap_oe_stp2))}
  
  proco=c("thrombin",'phylloquinone', 'menadione','desmopressin', 'U46619')
  
  cols[grepl(paste(toupper(proco), collapse = '|'),toupper(colnames(stp)),ignore.case = T)]= 'red4'
  cols_ilincs_Con[grepl(paste(toupper(proco), collapse = '|'),toupper(colnames(conndata_stp)),ignore.case = T)]= 'red4'
  cols_cmap_kd_Con[grepl(paste(toupper(proco), collapse = '|'),toupper(colnames(conndata_cmap_kd_stp)),ignore.case = T)]= 'red4'
  cols_cmap_oe_Con[grepl(paste(toupper(proco), collapse = '|'),toupper(colnames(conndata_cmap_oe_stp)),ignore.case = T)]= 'red4'
  if (!is.null(conndata_stp2)|!is.null(conndata_cmap_kd_stp2)|!is.null(conndata_cmap_oe_stp2)) {
    cols_ilincs_Con2[grepl(paste(toupper(proco), collapse = '|'),toupper(colnames(conndata_stp2)),ignore.case = T)]= 'red4'
    cols_cmap_kd_Con2[grepl(paste(toupper(proco), collapse = '|'),toupper(colnames(conndata_cmap_kd_stp2)),ignore.case = T)]= 'red4'
    cols_cmap_oe_Con2[grepl(paste(toupper(proco), collapse = '|'),toupper(colnames(conndata_cmap_oe_stp2)),ignore.case = T)]= 'red4'}
  heatmaps=list()
  
  lgd_list=Legend(labels = c("Pro-coagulants", "Other drugs"), title = "Compound types", type = "points", pch = 15, legend_gp = gpar(col = c("red4","black"), fontsize=48),grid_height = unit(0.3, "cm"),grid_width = unit(0.3, "cm"),labels_gp =gpar(fontsize = 15, fontface = "bold"),title_gp = gpar(fontsize = 15, fontface = "bold") )
  
  #Generate a heatmap annotation with our reference data, of provided
...
jpeg(filenames, units="in", width=width_heat, height=height_heat, res=res_heat)  
  ...
  if (!is.null(conndata_stp2)|!is.null(conndata_cmap_kd_stp2)|!is.null(conndata_cmap_oe_stp2)) {
    ...
    #Store the heatmaps as a list in vertical order
...
#Sometimes you may end up with appended names of strings with a delimiter on each cell, and you may want to retrieve each individual name and count them. freqfunc2 function handles that.
