...
## (if knock-down study; genes with negative connectivity score (<(-97), or if overexpression; genes with positive connectivity score above the threshold), 
...
ptrn='_conn_HA1E_'; types=c('kd','oe')
...
hepg2_conndata=connectivitydatafile_compiler(ptrn = ptrn,types = types, cmaprank_cutoff = 98,cor_outputs = T, heatmap_outputs = T)
...
hepg2_conndata[grepl(paste(candygenes, collapse = '|'),hepg2_conndata$Name_type,ignore.case = T),grepl(paste(drugs, collapse = '|'), colnames(hepg2_conndata), ignore.case = T)]
...
kegg_data_summ=enrichr_output(drug_list = drug_list ,dataset = hepg2_conndata,dbs = alt_dbs[1],cmapranks = T,cmaprank_cutoff = 98)
...
kegg_mestwar_summ=slicenricher(enrichrlist = kegg_data_summ,combscore_cutoff = 100,cor_outputs = T,heatmap_outputs = T,cmapranks = T,refcompounds = drug_list,enrichmentdb = 'KEGG')
...
updownfunk=slicenricher_updown(enrichrlist = kegg_data_summ,combscore_cutoff = 100,contr1 = c('menadione', 'phylloquinone'), contr2 = c('warfarin', 'skatole', 'argatroban'), classContr1 = 'Pro-coagulants',classContr2 = 'Anti-coagulants',enrichmentdb = 'KEGG',repositoryName = 'iLINCS', cmapranks = T)
...
#Upon inspecting the upsetplot for the demo data here, Panthotenate and CoA biosynthesis might be an interesting pathway that moves different directions between menadione and argatroban
